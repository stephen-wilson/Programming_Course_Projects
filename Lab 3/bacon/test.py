...
# MY TESTS
def test_acted_together():
    actor1 = 1232763 # Shii Ann Huang
    actor2 = 6541 # Philip Bosco
    actor3 = 572600 # Sten Hollstrom
    actor4 = 109625 # Robert Viharo
    
    assert lab.acted_together(db_small, actor1, actor2) is True
    assert lab.acted_together(db_small, actor3, actor4) is False
    
def test_tiny_bacon_0():
    expected = {4724}
    res = lab.actors_with_bacon_number(db_tiny, 0)
    assert (expected == res), f"expected {expected}, got {res} for 0 bacon"

def test_tiny_bacon_1():
    expected = {2876, 1532}
    res = lab.actors_with_bacon_number(db_tiny, 1)
    assert (expected == res), f"expected {expected}, got {res} for 1 bacon"

def test_tiny_bacon_2():
    expected = {1640}
    res = lab.actors_with_bacon_number(db_tiny, 2)
    assert (expected == res), f"expected {expected}, got {res} for 2 bacon"

def test_tiny_bacon_3():
    expected = set()
    res = lab.actors_with_bacon_number(db_tiny, 3)
    assert (expected == res), f"expected {expected}, got {res} for 3 bacon"   

def test_tiny_bacon_path():
    expected = [4724, 2876, 1640]
    res = lab.bacon_path(db_tiny, 1640)
    assert (expected == res), f"expected {expected}, got {res} for ID 1640"
