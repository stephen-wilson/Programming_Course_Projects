...
def readmap(path):
    with open(path, "r") as txt:
        ...
        levelmap = [l.ljust(width, " ") for l in lines]
        return levelmap
...
def partincluded(r1, r2):
    return (
        ...
        or r1.y < r2.y < r2.y + r2.h < r1.y + r1.h
    )


def validpair(r1, r2):
    return (
        (not partincluded(r1, r2))
        and (not partincluded(r2, r1))
        and (
            (2 * r1.x + r1.w != 2 * r2.x + r2.w) or (2 * r1.y + r1.h != 2 * r2.y + r2.h)
        )
    )
...
def rectangle_pairs():
    ...
    randomized = [
        lab.Rectangle(x, y, w, h) for (x, y, w, h) in randomized if w > 0 and h > 0
    ]
...
def run_intersection(test):
    return [
        (serialize_rect(r1), serialize_rect(r2), r1.intersects(r2))
        for r1, r2 in rectangle_pairs()
    ]


def run_translation_vector(test):
    return [
        (
            serialize_rect(r1),
            serialize_rect(r2),
            lab.Rectangle.translation_vector(r1, r2),
        )
        for r1, r2 in rectangle_pairs()
    ]
...
def run_test(input_data):
    ...
    try:
        testfn = {
            ...
            "translation_vector": run_translation_vector,
        }[test_type]
...
class InstrumentedGame(object):
    ...
    def add_rect_field(blobs):
        for b in blobs:
            if "pos" in b:
                b["rect"] = list(b.pop("pos")) + [lab.TILE_SIZE, lab.TILE_SIZE]
        return blobs

    @staticmethod
    def adjust_blobs(blobs):
        assert all(
            ("rect" in b or "pos" in b) and "texture" in b and "player" in b
            for b in blobs
        )
...
def init_game(levelname):
    ...
    player_x = 0
    return current_game.ghost_enabled
...
def timestep(args):
    ...
    return status, result, current_game.step
