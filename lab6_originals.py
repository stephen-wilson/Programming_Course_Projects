...
def num_neighbor_bombs(num_rows, num_cols, board, row_idx, col_idx):
    ...
    """
    valid_row_idx = lambda r: 0 <= r < num_rows 
    valid_col_idx = lambda c: 0 <= c < num_cols
    ...
    for r in range(row_idx - 1,row_idx + 2):
        if valid_row_idx(r):
            for c in range(col_idx - 1, col_idx + 2):
                if valid_col_idx(c):
                    num_bombs += board[r][c] == '.' # add 1 to num bombs if bomb at that location
    return num_bombs
...
def new_game_2d(num_rows, num_cols, bombs):
    ...
    # board = []
    # hidden = []
    # bombs_set = set(bombs) # can't be multiple bombs at same location
    ...
    #     for c in range(num_cols):
    #         if board[r][c] != '.':
    #             board[r][c] = num_neighbor_bombs(num_rows,num_cols,board,r,c)
    # return {
    #     "dimensions": (num_rows, num_cols),
    #     "board": board,
    #     "hidden": hidden,
...
# def new_game_2d_original(num_rows, num_cols, bombs):
#     """
#     Start a new game.
...
def dig_2d(game, row, col):
    ...
    # # change hidden to False (unhide) if it was hidden and set revealed to 1, otherwise return 0
